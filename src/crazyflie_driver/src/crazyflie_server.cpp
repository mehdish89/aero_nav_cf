...
public:
    ...
    , m_sentSetpoint(false)
  {
    ...
    //m_subscribeCmdVel = n.subscribe("/randomly_topic", 1, &CrazyflieROS::cmdVelChanged, this);
    ...
    //m_pubImu = n.advertise<sensor_msgs::Imu>(tf_prefix + "/base_link", 10);
    ...
    m_pubStab = n.advertise<geometry_msgs::PoseStamped>(tf_prefix + "/stabilizer", 10);
...
private:
  ...
  void cmdVelChanged(
    const geometry_msgs::Twist::ConstPtr& msg) //cmd_vel
  {
    if (!m_isEmergency) {
  ...
  void onImuData(logImu* data) {
    ...
    // msg.angular_velocity.x = data->gyro_x * 9.81;
    // msg.angular_velocity.y = data->gyro_y * 9.81;
    // msg.angular_velocity.z = data->gyro_z * 9.81;
  ...
  void onLog2Data(log2* data) {
...

    {
      ...
      msg.header.frame_id = m_tf_prefix + "/base_link";
      // measured in Tesla
      msg.pose.position.x = data->roll;
      msg.pose.position.y = data->pitch;
      msg.pose.position.z = data->yaw;
      m_pubStab.publish(msg);
    }
